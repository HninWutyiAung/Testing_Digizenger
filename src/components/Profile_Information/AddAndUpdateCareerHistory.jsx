...
const AddCareerHistory = ({ isOpenCar, onClose, onSave, currentCareer }) => {
    ...
    useEffect(() => {
        if (currentCareer) {
            ...
            setCompany(currentCareer.company || '');
            setStartDate(currentCareer.startDate || '');
            setEndDate(currentCareer.endDate === 'Present' ? '' : currentCareer.endDate || '');
            setCurrentlyWorking(currentCareer.endDate === 'Present');
        }
    }, [currentCareer]);
    ...
    const handleSubmit = (e) => {
        ...
        if (validateForm()) {
            const careerData = {
                id: currentCareer?.id || Date.now(), 
                designation,
                company,
                startDate,
                endDate: currentlyWorking ? 'Present' : endDate,
                logoUrl: currentCareer ? currentCareer.logoUrl : 'https://via.placeholder.com/60x60',
            };
            onSave(careerData); 
            onClose();
        }
